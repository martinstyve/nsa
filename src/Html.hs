...
indexMaybeError maybeError = do
  html_ do
    ...
    body_ do
      div_ [class_ "card"] do
        h1_ "Norwegian Singles Running Calculator"
...
trainingSection pace = tr_ do
  ...
  where
    minText = RT.showRunTime (RT.secToRunTime (minPace pace))
    maxText = RT.showRunTime (RT.secToRunTime (maxPace pace))
